            ...
            try {
                ...
                if (resource === 'booking' && operation === 'get') {
                    ...
                    if (filters.propertyId) {
                        const propertyIds = filters.propertyId.split(',').map(id => id.trim());
                        queryParams.push(`propertyId=${propertyIds.join(',')}`);
                    }
                    if (filters.roomId) {
                        const roomIds = filters.roomId.split(',').map(id => id.trim());
                        ...
                        try {
                            ...
                            try {
                                ...
                                if (!this.continueOnFail()) {
                                    throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Failed to parse JSON for booking ${bookingId}: ${errorMessage}. Please check your JSON syntax.`, {
                            ...
                            const errorMessage = error instanceof Error ? error.message : String(error);
                            if (!this.continueOnFail()) {
                                throw new n8n_workflow_1.NodeOperationError(this.getNode(), `Failed to ${operation} ${resource}: ${errorMessage}`);
                            }
                            const executionErrorData = this.helpers.constructExecutionMetaData(this.helpers.returnJsonArray({ error: errorMessage }), { itemData: { item: i } });
                            returnData.push(...executionErrorData);
                            continue;
...
exports.Beds24 = Beds24;
//# sourceMappingURL=Beds24.node.js.map