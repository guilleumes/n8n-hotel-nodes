...
export class Beds24Paginated implements INodeType {
...
// --- FIN PROPERTIES ---
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> { // --- INICIO EXECUTE ---
	...
	for (let i = 0; i < items.length; i++) {
		try {
			const credentials = await this.getCredentials('beds24Api') as IDataObject;

			if (resource === 'booking' && operation === 'get') {
				// ... igual que el nodo original ...
				const bookingId = this.getNodeParameter('bookingId', i) as string;
				const additionalFields = this.getNodeParameter('additionalFields', i, {}) as IDataObject;
				let queryParams = `?id=${bookingId}`;
				if (additionalFields.includeInvoiceItems) queryParams += '&includeInvoiceItems=true';
				if (additionalFields.includeInfoItems) queryParams += '&includeInfoItems=true';
				const options: IHttpRequestOptions = {
					headers: { 'token': credentials.token as string },
					method: 'GET',
					baseURL: 'https://api.beds24.com/v2',
				...
				let queryParams = [];
				if (filters.filter) queryParams.push(`filter=${filters.filter}`);
				if (filters.propertyId) queryParams.push(`propertyId=${filters.propertyId}`);
				...
				for (const dateFilter of dateFilters) {
					if (filters[dateFilter]) queryParams.push(`${dateFilter}=${filters[dateFilter]}`);
				}
				if (filters.searchString) queryParams.push(`searchString=${filters.searchString}`);
				// Usa los parámetros principales en vez de filters
				const includeInvoiceItems = this.getNodeParameter('includeInvoiceItems', i, false) as boolean;
				const includeInfoItems = this.getNodeParameter('includeInfoItems', i, true) as boolean;
				if (includeInvoiceItems) queryParams.push('includeInvoiceItems=true');
				if (includeInfoItems) queryParams.push('includeInfoItems=true');

				let allData: any[] = [];
				let nextPageUrl: string | null = '/bookings' + (queryParams.length > 0 ? '?' + queryParams.join('&') : '');
				const baseURL = 'https://api.beds24.com/v2';
				const headers = { 'token': credentials.token as string };
				...
				// ... igual que el nodo original ...
				const bookingId = this.getNodeParameter('bookingId', i) as string;
				const updateMethod = this.getNodeParameter('updateMethod', i) as string;
				let updateData: IDataObject;
				if (updateMethod === 'raw') {
					const rawData = this.getNodeParameter('rawUpdateData', i) as string;
					...
					if (updateFields.firstName || updateFields.lastName) updateData.guestName = `${updateFields.firstName || ''} ${updateFields.lastName || ''}`.trim();
					if (updateFields.email) updateData.guestEmail = updateFields.email;
					if (updateFields.phone) updateData.guestPhone = updateFields.phone;
					if (updateFields.notes) updateData.notes = updateFields.notes;
					if (updateFields.arrival) updateData.arrival = updateFields.arrival;
					if (updateFields.departure) updateData.departure = updateFields.departure;
				}
				const options: IHttpRequestOptions = {
					headers: {
